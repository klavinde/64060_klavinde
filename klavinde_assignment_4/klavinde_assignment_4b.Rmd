...
### The WSS elbow is more gradual than the example given in class however it appears that 5 or 6 clusters would be appropriate. 

### The Silhouette identifies 5 clusters being optimal.

### Continue with using 5 clusters
```{r}
...
##Tracing all of the groups back to the .csv table, there does not appear to be a consistent Median Recommendation in the clusters.
...
for (i in c(1:9))
lines(k5$centers[,i], lty = i, lwd = 2, col = ifelse(i %in% c(0,1),
                            "black", "dark grey"))
text(x = 0.2, y = k5$centers[, 1], labels = paste("Cluster", c(1:5)))
```

### Notebook wont put the lines if "lines(k5$centers[i,]," but they appear correct on RStudio. Line 86 has an unknown error but plots something if re-written as above. 
